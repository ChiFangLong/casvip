...
public class activitiHistoryYtil {
	...
	public static String getProcessCreator(String processInsId) {
		return historyService.createHistoricProcessInstanceQuery().processInstanceId(processInsId).singleResult().getStartUserId();
	}
	...
	public static List<HistoricProcessInstance> getMyRequestProcess(String userId, String searchType) {
		if("已完成".equals(searchType)){
			return historyService.createHistoricProcessInstanceQuery().finished().startedBy(userId).list();
		} else if("未完成".equals(searchType)){
			return historyService.createHistoricProcessInstanceQuery().unfinished().startedBy(userId).list();
		}
		return historyService.createHistoricProcessInstanceQuery().startedBy(userId).list();
	}
	
	public static List<HistoricProcessInstance> getMyRequestProcess(String userId, int firstNum, int maxNum) {
		return historyService.createHistoricProcessInstanceQuery().startedBy(userId).orderByProcessInstanceStartTime().desc().listPage(firstNum, maxNum);
	}
	public static List<HistoricProcessInstance> getMyRequestProcess(String userId, String searchType, int firstNum, int maxNum) {
		if("已完成".equals(searchType)){
			return historyService.createHistoricProcessInstanceQuery().startedBy(userId).finished().orderByProcessInstanceStartTime().desc().listPage(firstNum, maxNum);
		} else if("未完成".equals(searchType)){
			return historyService.createHistoricProcessInstanceQuery().startedBy(userId).unfinished().orderByProcessInstanceStartTime().desc().listPage(firstNum, maxNum);
		}
		return historyService.createHistoricProcessInstanceQuery().startedBy(userId).orderByProcessInstanceStartTime().desc().listPage(firstNum, maxNum);
	}
