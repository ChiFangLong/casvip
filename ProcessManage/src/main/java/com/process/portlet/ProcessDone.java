...
/**
 * Portlet implementation class ProcessDone
 */
public class ProcessDone extends MVCPortlet {
	
	JdbcTemplate jdbcTemplate = ActivitiUtil.getJdbcTemplate();
	...
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if (resourceType.equals("1")) {
			...
			Integer page = ParamUtil.getInteger(resourceRequest, "currPage");		// 当前页码数
			String searchType = ParamUtil.getString(resourceRequest, "searchType");	// 查询类型 (全部\未完成\完成)
			tbodyContent = ActivitiUtil.pageProcess(resourceRequest, screenName, page, searchType);
			out.print(tbodyContent);
		} else if (resourceType.equals("2")) {
			// 获取存入的参数
			Integer setTotalCount = (Integer) resourceRequest.getPortletSession().getAttribute("setTotalCount",
					PortletSession.APPLICATION_SCOPE);// 数据总数量
			Integer totalPages = (Integer) resourceRequest.getPortletSession().getAttribute("totalPages",
					PortletSession.APPLICATION_SCOPE);// 数据总页数
			// 存入map
	...
	public void goToReadRequestProcessAct(ActionRequest actionRequest, ActionResponse actionResponse) {
		...
		actionRequest.getPortletSession().setAttribute("processFormForRequest", processForm,
				PortletSession.APPLICATION_SCOPE);
		actionResponse.setRenderParameter("mvcPath", "/html/processdone/HistoryProcView.jsp");
	}
	...
	public void goProcessPngJSP(ActionRequest actionRequest, ActionResponse actionResponse) {
		...
		// 存入session 供 前端获取
		actionRequest.getPortletSession().setAttribute("byteList", JSON.toJSONString(lists),
				PortletSession.APPLICATION_SCOPE);
	}
